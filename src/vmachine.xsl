...
<xsl:output method="html" version="4.01" encoding="utf-8" indent="yes" doctype-system="http://www.w3.org/TR/html4/strict.dtd" doctype-public="-//W3C//DTD HTML 4.01//EN" />
   ...
   <!-- edited by Theodore Koterwas 10/06/2011: set a variable panZoomImage
        and javascript includes for adding pan and zoom functionality via OpenLayers
   -->
   <xsl:variable name="indexOfTexts">false</xsl:variable>
   <xsl:variable name="panZoomImage">true</xsl:variable>
   <xsl:variable name="useBrandingLogo">false</xsl:variable>
   <xsl:variable name="OpenLayersInclude">http://openlayers.org/api/OpenLayers.js</xsl:variable>
   <xsl:variable name="panZoomImageInclude">../src/panZoomImage.js</xsl:variable>
   <xsl:variable name="jQueryHeaderStuff"></xsl:variable>
   <xsl:variable name="singleWitOnClick"></xsl:variable>
   <xsl:variable name="navigationStuff"></xsl:variable>
   <xsl:variable name="getApp">true</xsl:variable>
   <!-- end Theodore Koterwas edit -->
   <xsl:variable name="matchLinesHighlight">false</xsl:variable>
  ...
  "true" to "false" below -->
   ...
   <xsl:template match="/">
      <html xml:lang="en">
         ...
         <body onload="init();">
            <!-- edited by Theodore Koterwas 28/02/2012: wrapped vm stuff in a div to 
            more easily allow for left column  and wrapped everything below main navigation
            in a wrapper dive for easier theming -->
            <xsl:call-template name="mainHeading"/>
            <xsl:copy-of select="$mainNavigation"/>
            <div id="main">
	            <xsl:copy-of select="$navigationStuff"/>
	            <div id="versioningMachine">
            ...
            <!-- end edited by Theodore Koterwas 28/02/2012 -->
   ...
   <xsl:template name="htmlHead">
      <head>
         ...
         <!-- edited by Theodore Koterwas 10/06/2011: test panZoomImage
              and include the appropriate javascript for panning and zooming
         -->
        <xsl:if test="$panZoomImage = 'true'">
         ...
         <xsl:copy-of select="$jQueryHeaderStuff"/>
        <!-- end Theodore Koterwas edit -->
      </head>
   </xsl:template>
   ...
   <xsl:template name="mainBanner">
      <div id="mainBanner">
    <!-- commented out by Theodore Koterwas 09/29/2011 -->
        <!--<xsl:call-template name="headline" />-->
    <!-- end commented out by Theodore Koterwas 09/29/2011 -->
        <xsl:call-template name="mainControls" />
      </div>
   </xsl:template>

   <!--added by Theodore Koterwas 06/10/2011 -->
   <xsl:template name="mainHeading">
      <div id="mainHeading">
          <div id="mainHeadingTitle">
            The Wandering Jew's Chronicle
         </div>
         <div id="mainHeadingSubTitle">
         </div>
      </div>
   </xsl:template>
   <!--end added by Theodore Koterwas 06/10/2011 -->
   
   <xsl:template name="brandingLogo">
<xsl:if test="$useBrandingLogo='true'">
      <div id="brandingLogo">
            ...
            <xsl:for-each select="$witnesses">
               ...
               <xsl:for-each select="//tei:note[@type='image']/tei:witDetail[@target = concat('#',$witID)]//tei:graphic[@url]">
                  <xsl:call-template name="imageLink">
                     ...
                     <xsl:with-param name="witness" select="translate(translate(ancestor::tei:witDetail/@wit,'#',''), '_', ' ')" />
                  </xsl:call-template>
               </xsl:for-each>
            </xsl:for-each>
               ...
               <div>
                  ...
                  <xsl:choose>
                     <xsl:when test="ancestor::tei:l">
                        <div class="position">
<xsl:if test="not($matchLinesHighlight='false')">
                           ...
                           <xsl:choose>
                              ...
                              <xsl:when test="ancestor::tei:l//tei:milestone[@unit='line']">
                                 <xsl:text>Line number </xsl:text>
                                 <xsl:value-of select="ancestor::tei:l//tei:milestone[@unit='line']/@n" />
                              </xsl:when>
                              <xsl:otherwise>
...
<xsl:value-of select="concat(name(), $thingID)"/></xsl:attribute>
   ...
   <xsl:template name="imageLink">
      ...
      <!-- edited by Theodore Koterwas 10/06/2011: to add folio and folioString parameters
           to be passed showImgPanel
      -->
      
      <xsl:param name="folio"/>
      <!-- xsl:choose added by James Cummings 2011-05-03 to enable folio-references instead of image -->
      <xsl:param name="folioString"/>
      <!-- end edited by Theodore Koterwas 10/06/2011 -->
      ...
      <xsl:if test="$imageURL != ''">
         <img src="../vm-images/image.gif">
            ...
            <xsl:attribute name="onclick">
               ...
               <xsl:text>','</xsl:text>
               <xsl:value-of select="$witness" />
               <!-- edited by Theodore Koterwas 10/06/2011: to pass the folioString, folio, and panZoomImage
...
<xsl:value-of select="concat('line', $lineID)"/></xsl:attribute>
         <xsl:attribute name="class">
            <xsl:text>line</xsl:text>
            <xsl:text> line</xsl:text>
            <xsl:value-of select="$uniqueID" />
         </xsl:attribute>
<xsl:if test="not($singleWitOnClick='false') and not($matchLinesHighlight='false')">
         <xsl:attribute name="onclick">
            <xsl:text>matchLine('line</xsl:text>
            <xsl:value-of select="$uniqueID" />
            <xsl:text>');</xsl:text>
...
<span class="linenumber">
   ...
   getLine('app-4')
   -->
   <xsl:if test="$getApp='true'">
      <xsl:attribute name="onclick">
         <xsl:variable name="appOrLine">Line</xsl:variable>
            <xsl:variable name="lineAppOrHead">line</xsl:variable>
         <xsl:variable name="numberOrID"><xsl:value-of select="concat('appline', @xml:id)"/></xsl:variable>
         <xsl:variable name="getStuff">$(this).getLine('<xsl:value-of select="$numberOrID"/>');</xsl:variable>
         <xsl:value-of select="normalize-space($getStuff)"/>
         <!-- <xsl:text>getApp('app-</xsl:text>
...
-->
      ...
      <xsl:for-each select=".//*[@facs]">
         <xsl:call-template name="imageLink">
            ...
            <xsl:with-param name="witness">
               <xsl:choose>
                  <xsl:when test="@ed">
                     <xsl:value-of select="translate(translate(@ed,'#',''), '_', ' ')" />
                  </xsl:when>
                  <xsl:when test="ancestor::*/@wit">
                     <xsl:value-of select="translate(translate(ancestor::*/@wit,'#',''), '_', ' ')" />
                  </xsl:when>
               </xsl:choose>
            </xsl:with-param>
         </xsl:call-template>
         ...
         <xsl:call-template name="imageLink">
            ...
            <xsl:with-param name="witness">
               <xsl:choose>
                  <xsl:when test="@ed">
                     <xsl:choose>
                        <xsl:when test="contains(@ed, '.jpg')">
                     <xsl:value-of select="substring-before(translate(translate(@ed,'#',''), '_', ' '), '.jpg')" />
                        </xsl:when>
                        <xsl:otherwise>
                           <xsl:value-of select="translate(translate(@ed,'#',''), '_', ' ')" />
                        </xsl:otherwise>
   ...
   <xsl:template match="tei:app">
      ...
      <span>
         ...
         <xsl:if test="$getApp='true'">
            <xsl:attribute name="onclick">
	    <xsl:variable name="appOrLine"><xsl:choose>
<xsl:when test=".//tei:milestone[@unit='line']">Line</xsl:when><xsl:otherwise>App</xsl:otherwise></xsl:choose></xsl:variable>
	    <xsl:variable name="lineAppOrHead"><xsl:choose>
<xsl:when test="$appOrLine='Line' and ancestor::tei:l">line</xsl:when><xsl:when test="$appOrLine='Line' and ancestor::tei:head">head</xsl:when><xsl:otherwise>app</xsl:otherwise></xsl:choose></xsl:variable>
            <xsl:variable name="numberOrID">
<xsl:choose>
             <xsl:when test="$lineAppOrHead='app'"><xsl:value-of select="count(preceding::tei:app)+1"/></xsl:when>
             <xsl:when test="$lineAppOrHead='head'"><xsl:value-of select="concat('app',$lineAppOrHead, ancestor::tei:head/@xml:id)"/></xsl:when>
             <xsl:when test="$lineAppOrHead='line'"><xsl:value-of select="concat('app',$lineAppOrHead, ancestor::tei:l/@xml:id)"/></xsl:when>
</xsl:choose>
</xsl:variable>
<xsl:variable name="getStuff"><xsl:value-of select="concat('$(this).get', $appOrLine)"/>('<xsl:if test="$lineAppOrHead='app'"><xsl:value-of select="$lineAppOrHead"/>-</xsl:if><xsl:value-of select="$numberOrID"/>');</xsl:variable>
<xsl:value-of select="normalize-space($getStuff)"/>
           <!-- <xsl:text>getApp('app-</xsl:text>
            <xsl:value-of select="count(preceding::tei:app)+1" />
            <xsl:text>');</xsl:text>-->
